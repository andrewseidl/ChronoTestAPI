...
def validateTest(data):
    ...
    for key in keys:
        if(not key in data):
            message = "Missing " + key + \
                " argument provided. Required to create a test"
            abort(400)
...
class TestAPI(Resource):
    ...
    def put(self, test_name):
        ...
        # update arguments in test. In here we want to add one more data point
        # to the test. Either if it passed or failed.
...
api.add_resource(
    TestAPI, '/chrono_test/api/tests/<string:test_name>', endpoint='test')
